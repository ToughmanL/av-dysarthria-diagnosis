...
class TSNSSEGTSPLIT(nn.Module):
  def __init__(self, num_class, out_dim, num_segments, modality='RGB',
    ...
    super(TSNSSEGTSPLIT, self).__init__()
    ...
    self.conv = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=1, stride=1, padding=0, bias=False)
  ...
  def forward(
    ...
    if self.modality_type == 'twostream' or self.modality_type == 'space':
      ...
      # rgb_out_list.append(self.tns_space_2(speech, rgb_data[1]).unsqueeze(1))
      # rgb_out_list.append(self.tns_space_3(speech, rgb_data[2]).unsqueeze(1))
      # rgb_out_list.append(self.tns_space_4(speech, rgb_data[3]).unsqueeze(1))
    if self.modality_type == 'twostream' or self.modality_type == 'time':
    ...
    if len(self.modality) == 2:
      output = self.conv(torch.stack([rgb_out, flow_out], dim=1)).squeeze(1)
      output = self.bn(output)
    elif rgb_out is not None:
      output = rgb_out
    else:
      output = flow_out
    # output = rgb_out+flow_out if len(self.modality) == 2 else rgb_out if rgb_out is not None else flow_out
