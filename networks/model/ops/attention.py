...
from networks.model.ops.MMD import MMDLoss
...
class CosineAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        batch_size, length, hidden_dim = query.size(0), query.size(1), query.size(2)
        score = F.cosine_similarity(query.view(batch_size * length, hidden_dim), key.view(batch_size * length, hidden_dim), dim=-1)
        score = score.view(batch_size, length, -1)
        if mask is not None:
            score.masked_fill_(mask.view(score.size()), -float('Inf'))
...
class MMDAttention(nn.Module):
    def __init__(self, dim: int):
        super(MMDAttention, self).__init__()
        self.sqrt_dim = np.sqrt(dim)
        self.mmd = MMDLoss()

    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        batch_size, length, hidden_dim = query.size(0), query.size(1), query.size(2)
        score = self.mmd(query.view(batch_size * length, hidden_dim), key.view(batch_size * length, hidden_dim))
        score = score.view(batch_size, length, -1)
        if mask is not None:
            score.masked_fill_(mask.view(score.size()), -float('Inf'))
...
class EuclideanAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        score = torch.cdist(query, key, p=2)
        if mask is not None:
            score.masked_fill_(mask.view(score.size()), -float('Inf'))
        # score = F.softmax(score, -1)
        context = torch.bmm(score, value)
...
class KLdivergenceAttention(nn.Module):
    def __init__(self, dim: int):
        super(KLdivergenceAttention, self).__init__()
        self.sqrt_dim = np.sqrt(dim)

    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        query = F.softmax(query, -1)
        key = F.softmax(key, -1)
        score = F.kl_div(query.log(), key, reduction='none').sum(-1, keepdim=True)
        if mask is not None:
            score.masked_fill_(mask.view(score.size()), -float('Inf'))
        # score = F.softmax(score, -1)
        context = torch.bmm(score, value)
...
class MultiHeadCustomAttention(nn.Module):
    def __init__(self, d_model: int = 512, num_heads: int = 8, similiarity: str = 'scaled_dot'):
        super(MultiHeadCustomAttention, self).__init__()
        ...
        self.num_heads = num_heads
        if similiarity == 'none':
            self.attn = DotProductAttention(self.d_head)
        elif similiarity == 'scaled_dot':
            self.attn = ScaledDotAttention(self.d_head)
        elif similiarity == 'cosine':
            self.attn = CosineAttention(self.d_head)
        elif similiarity == 'euclidean':
            self.attn = EuclideanAttention(self.d_head)
        elif similiarity == 'kl_divergence':
            self.attn = KLdivergenceAttention(self.d_head)
        elif similiarity == 'mmd':
            self.attn = MMDAttention(self.d_head)
    ...
    def forward(
        ...
        context, attn = self.attn(query, key, value, mask)
...
A_un = torch.unsqueeze(A, 1) # B, C -> T, B, C
B_un = torch.unsqueeze(B, 1) # B, C -> T, B, C
multiheadatt = nn.MultiheadAttention(20, 1, batch_first=True)
multihead_output, multihead_weights = multiheadatt.forward(A_un, B_un, B_un)
multiHeadatt = MultiHeadCustomAttention(20, 1, 'mmd')
multiHead_output, multiHead_weights = multiHeadatt.forward(A_un, B_un, B_un)

scaledotatt = ScaledDotProductAttention(20)
scaledotatt_output, scaledotatt_weights = scaledotatt.forward(A_un, B_un, B_un)
print(multihead_output.shape, multihead_weights.shape)
print('done')