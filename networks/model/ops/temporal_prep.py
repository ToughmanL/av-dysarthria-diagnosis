...
# -*- encoding: utf-8 -*-
'''
 *@File	:	temporal_prep.py
 *@Time	: 2024-04-14 17:50:02
 *@Author	:	lxk
 *@Version	:	1.0
...
# compute rgb diff
def get_rgbdiff(input, new_length, keep_rgb=False):
    new_data = input.clone()
    for x in reversed(list(range(1, new_length))):
      new_data[:, x - 1, :, :, :] = input[:, x, :, :, :] - input[:, x - 1, :, :, :]
...
# compute rgb diff
def get_rgbdiff(input, new_length, keep_rgb=False):
    # input: (batch_size, segmentnum, length, channel, height, width)
    new_data = input.clone()
    # segment loop
    for i in range(input.size(1)):
        segment_length = input.size(2)
        # frame loop
        for x in range(1, segment_length):
            new_data[:, i, x, :, :, :] = input[:, i, x, :, :, :] - input[:, i, x - 1, :, :, :]
        if i == 0: # 第一个片段的第一帧取0
            new_data[:, i, 0, :, :, :] = 0
        else: # 其他片段的第一帧取前一个片段的最后一帧
            new_data[:, i, 0, :, :, :] = input[:, i, 0, :, :, :] - input[:, i-1, -1, :, :, :]
    return new_data
'''


# 计算光流
def get_flow(input, new_length,method='DualTVL1'):
    flow_data = []
    while True:
        ret, frame2 = cap.read()
        if not ret:
