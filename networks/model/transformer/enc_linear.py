...
class EncLinear(torch.nn.Module):
    ...
    def forward(
        ...
        assert (speech.shape[0] == speech_lens.shape[0]), (speech.shape, speech_lens.shape)
        # 1. Encoder
        if self.model_type == "former":
            ...
            # encoder_out = encoder_out.mean(dim=1) # mean
            encoder_out = encoder_out.max(dim=1)[0] # max
        ...
        elif self.model_type == "resnet3d":
            encoder_out = self.encoder(speech, video, speech_lens)
