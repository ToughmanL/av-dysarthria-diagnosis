...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.batch_norm2 = nn.BatchNorm1d(self.stats_dim * block.expansion * 2)
    ...
    def forward(self, x, x_len):
        ...
        stats = self.pool(out)
        stats = self.batch_norm2(stats)

        embed_a = self.seg_1(stats)
        embed_a = self.batch_norm3(embed_a)
        # if self.two_emb_layer:
        #     out = F.relu(embed_a)
...
def ResNet10(feat_dim, embed_dim, pooling_func='ASP', two_emb_layer=False):
...
def ResNet18(feat_dim, embed_dim, pooling_func='ASP', two_emb_layer=False):
...
def ResNet34(feat_dim, embed_dim, pooling_func='ASP', two_emb_layer=False):
...
def get_resnet(resnum, feat_dim, embed_dim, pooling_func='ASP', two_emb_layer=False):
    if resnum == 10:
        return ResNet10(feat_dim, embed_dim, pooling_func, two_emb_layer)
    elif resnum == 18:
        return ResNet18(feat_dim, embed_dim, pooling_func, two_emb_layer)
    elif resnum == 34:
        return ResNet34(feat_dim, embed_dim, pooling_func, two_emb_layer)
